...
int
u2kcopy(pagetable_t user,pagetable_t kernel,uint64 lo,uint64 hi)
{
  uint64 va;
  pte_t *upte;
  pte_t *kpte;
...
void vmprint_helper(pagetable_t pagetable,int leval)
{
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
      for(int j=0;j<leval;j++)
      {
          printf(".. ");
      }
      printf("..");
      ...
      if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
        // this PTE points to a lower-level page table.
        vmprint_helper((pagetable_t)child,leval+1);
      }
    }
  }
}
